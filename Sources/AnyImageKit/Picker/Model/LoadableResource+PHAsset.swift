...
extension PHAsset: LoadableResource {
    ...
    public func loadResource(type: MediaType, completion: (Result<MediaResourceStorage, AnyImageError>) -> Void) {
        
    }
}

extension LoadableResource where Self: PHAsset {
    
    func loadLibraryPhoto(options: LibraryPhotoLoadOptions, completion: @escaping LibraryPhotoLoadCompletion) -> Int {
    ...
    func loadLibraryPhotoData(options: PhotoDataLoadOptions, completion: @escaping LibraryPhotoDataLoadCompletion) -> Int {
        let phRequestOptions = PHImageRequestOptions()
        phRequestOptions.version = options.version
    ...
    func loadLibraryLivePhoto(options: LivePhotoLoadOptions, completion: @escaping LibraryLivePhotoLoadCompletion) -> Int {
    ...
    func loadLibraryVideo(options: LibraryVideoLoadOptions, completion: @escaping LibraryVideoLoadComletion) -> Int {
        ...
        let requestID = PHImageManager.default().requestAVAsset(forVideo: self, options: phRequestOptions) { (avAsset, avAudioMix, info) in
            if let avAsset = avAsset, let avAudioMix = avAudioMix, let requestID = (info?[PHImageResultRequestIDKey] as? PHImageRequestID) {
                completion(.success(.init(identifier: identifier, requestID: Int(requestID), avAsset: avAsset, avAudioMix: avAudioMix)))
            } else {
    ...
    func loadLibraryVideoData(options: LibraryVideoDataLoadOptions, completion: @escaping LibraryVideoDataLoadCompletion) -> Int {
